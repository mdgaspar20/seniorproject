...
$fname = $lname= $deptNum = $employeeLogin = $password = $role = "";
$fname_e = $lname_e= $deptNum_e= $employeeLogin_e = $password_e = $role_e = "";
...
if(isset($_POST["EmployeeID"]) && !empty($_POST["EmployeeID"])){
	...
	if(empty($password)){
		$password_e="Please enter password";
	}
    ...
    // Check input errors before inserting in database
     if(empty($fname_e) && empty($lname_e) && empty($deptNum_e) && empty($employeeLogin_e) && empty($role_e) && empty($password_e)){
        // Prepare an update statement
        $sql = "UPDATE employee SET Fname=?, Lname=?, DeptNum=?, EmployeeLogin=?, Password=?, Role=? WHERE EmployeeID=?";
         
        if($stmt = mysqli_prepare($conn, $sql)){
            // Bind variables to the prepared statement as parameters
			mysqli_stmt_bind_param($stmt, "ssssssi", $param_fname, $param_lname, $param_deptNum, $param_login, $param_pass, $param_role, $param_id);
...
<body>
	...
	<div class="form-group <?php echo (!empty($password_e)) ? 'has-error' : ''; ?>">
